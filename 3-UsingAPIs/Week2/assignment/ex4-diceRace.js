...
function rollDice() {
  ...
  const promises = dice.map(() => rollDie(die));
...
/*
Some dice continue rolling for some undetermined time after the promise returned by `Promise.race()` resolves because `Promise.race()` only waits for the first promise to resolve or reject. The other promises continue to execute in the background until they are resolved or rejected.
*/
